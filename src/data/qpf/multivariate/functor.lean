...
-/
import data.fin logic.function.basic
...
-- def append1 (α : typevec n) (β : Type*) : typevec (n+1) :=
-- by intro i; cases i with _ i; [exact α i, exact β]
...
-- recurse on the `fin' n` argument and not the `ℕ` argument. This
...
by intros n α p i; induction i; [apply i_ih (drop_fun p), exact _root_.subtype.val]
...
begin
  ...
  { clear x y _inst_2 _inst_1 F, dsimp [(⊚)],
    -- Lean 3.17.0 problem
    ext i x; induction i; rw [of_subtype'],
    { rw [to_subtype',i_ih], refl }, { dsimp [id], cases x, refl } },
